...
import pandas
summary_folder = r'Z:/ENN_acc/2_LwuTwu_iiX_em/'
# import requests
# import urllib3
...
if __name__ == '__main__':
    ...
    for i_trial in range (0,100):#take mean of result
        print('2_LwuTwu_iiX_em {}th trial!!!!!!!!!!!!!'.format(i_trial+1))
        ...
        #combine data: LwuTwu = Learning: mnist&fashion, Test: mnist&emnist
        #yu = 1 when U, 11D
        ...
        for mix_rate_l in mix_rate_ls:#loop for mix rate for learn data
            ...
            for mix_rate_t in mix_rate_ts:#loop for mix rate for test data
                ...
                x_learn_mix = np.concatenate([x_learn_m,x_learn_em_pick])
                y_learn_mix = np.concatenate([y_learn_m,y_learn_em_pick])
        ...
        accs_k_forMixlt_df.to_csv(summary_folder+r'{0}_accs_k_{1}.csv'.format('2_LwuTwu_iiX_em',i_trial))#index=mix_rate_t, column = mix_rate_learn
        accs_U_forMixlt_df.to_csv(summary_folder+r'{0}_accs_U_{1}.csv'.format('2_LwuTwu_iiX_em',i_trial))
        
        recalls_k_forMixlt_df.to_csv(summary_folder+r'{0}_recalls_k_{1}.csv'.format('2_LwuTwu_iiX_em',i_trial))#index=mix_rate_t, column = mix_rate_learn
        recalls_U_forMixlt_df.to_csv(summary_folder+r'{0}_recalls_U_{1}.csv'.format('2_LwuTwu_iiX_em',i_trial))
        
        precs_k_forMixlt_df.to_csv(summary_folder+r'{0}_precs_k_{1}.csv'.format('2_LwuTwu_iiX_em',i_trial))#index=mix_rate_t, column = mix_rate_learn
        precs_U_forMixlt_df.to_csv(summary_folder+r'{0}_precs_U_{1}.csv'.format('2_LwuTwu_iiX_em',i_trial))
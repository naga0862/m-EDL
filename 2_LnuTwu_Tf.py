...
import pandas
summary_folder = r'Z:/ENN_acc/2_LnuTwu/'
# import requests
# import urllib3
...
if __name__ == '__main__':
    ...
    for i_trial in range (0,100):#take mean of result
        print('2_LnuTwu {}th trial!!!!!!!!!!!!!'.format(i_trial+1))
        ...
        x_test_em, y_test_em = preprocess_images(x_test_em, y_test_em)#onehot vector y_test_em includes class10 = U
        
        #combine data: LnuTwu = Learning: mnist, Test: mnist&emnist
        #yu = 1 when U, 11D
        ...
        for mix_rate_t in mix_rate_ts:
            ...
            #model.fit(x_learn_m, y_learn_m, batch_size=64, shuffle=True,validation_split=0.1, epochs=1)#epochs=10
            ...
            y_pred_p_hat_pick_U = y_pred_phat[np.where(y_test_label == 10)]
            y_test_label_pick_U = y_test_label[np.where(y_test_label == 10)]
        ...
        accs_k_forMixt_df = pandas.DataFrame(accs_k_forMixt,index = mix_rate_ts)
        accs_U_forMixt_df = pandas.DataFrame(accs_U_forMixt,index = mix_rate_ts)
        
        accs_k_forMixt_df.to_csv(summary_folder+r'{0}_accs_k_{1}.csv'.format('2_LnuTwu',i_trial))
        accs_U_forMixt_df.to_csv(summary_folder+r'{0}_accs_U_{1}.csv'.format('2_LnuTwu',i_trial))
        
        recalls_k_forMixt_df = pandas.DataFrame(recalls_k_forMixt,index = mix_rate_ts)
        recalls_U_forMixt_df = pandas.DataFrame(recalls_U_forMixt,index = mix_rate_ts)
        
        recalls_k_forMixt_df.to_csv(summary_folder+r'{0}_recalls_k_{1}.csv'.format('2_LnuTwu',i_trial))
        recalls_U_forMixt_df.to_csv(summary_folder+r'{0}_recalls_U_{1}.csv'.format('2_LnuTwu',i_trial))
        
        precs_k_forMixt_df = pandas.DataFrame(precs_k_forMixt,index = mix_rate_ts)
        precs_U_forMixt_df = pandas.DataFrame(precs_U_forMixt,index = mix_rate_ts)
        
        precs_k_forMixt_df.to_csv(summary_folder+r'{0}_precs_k_{1}.csv'.format('2_LnuTwu',i_trial))
        precs_U_forMixt_df.to_csv(summary_folder+r'{0}_precs_U_{1}.csv'.format('2_LnuTwu',i_trial))